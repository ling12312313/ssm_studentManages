		...
		<form class="layui-form">
			<div class="layui-inline">
				<div class="layui-input-inline">
					<input type="text" class="layui-input searchValname" placeholder="请输入搜索的姓名" />
				</div>
				<div class="layui-input-inline">
...
<!-- 				<input type="text" class="layui-input searchValclazz" placeholder="请输入搜索的班级" />			
 ...
 -->			</div>
			...
			<div class="layui-inline">
				<a class="layui-btn layui-btn-normal addNews_btn">添加学生</a>
			</div>
			<div class="layui-inline">
...
<!-- <script type="text/javascript" src="newsList.js"></script>
 ...
 layui.use(['form','layer','laydate','table','laytpl'],function(){
       ...
       $.ajax({
			type: "get",
			url: "/ClazzServlet/getClazzList?t="+new Date().getTime()+"&from=ok",
			dataType:"json",
	            ...
	            {field:'id',title:'ID',width:50, lign:"center",sortable: true},    
 		        ...
 		        {field:'clazz_id',title:'班级',width:150
 		        	,templet: function(d){
 		        		...
 		        		$.ajax({
 		       			type: "get",
 		       			url: "/ClazzServlet/getClazzList?t="+new Date().getTime()+"&from=list",
 		       			dataType:"json",
 		       			async : false,//设置为同步
 		       			success:function(data){
 		       	          //  console.log(data);
	   ...
	   /*  //是否置顶
	    ...
	    $(".search_btn").on("click",function(){
	        if($(".searchValname").val() != ''||$("#userid").val()!=''){
	            ...
	            layer.msg("请输入搜索的内容");
	            table.reload("studentListTable",{ //再重新刷新表
	            	page: {
	                    curr: 1 //重新从第 1 页开始
	                },
	                where:{
	    ...
	    function editNews(edit){
	        var index = layui.layer.open({
	            title : "编辑用户",
	            type : 2,
	            content : "/SystemServlet/toStudentEdit",
	            success : function(layero, index){
	                ...
	                if(edit){
	                   ...
	                   // body.find("#sex").val(edit.sex);
	                    body.find(".edit_mobile").val(edit.mobile);
	                    body.find(".edit_qq").val(edit.qq);
	    ...
	    function addNews(edit){
	        var index = layui.layer.open({
	            title : "添加用户",
	            type : 2,
	            content : "/SystemServlet/toStudentAdd",
	            success : function(layero, index){
	                var body = layui.layer.getChildFrame('body', index);
	                if(edit){
	                    body.find(".newsName").val(edit.name);
	        ...
	        if(data.length > 0) {
	            ...
	            layer.confirm('确定删除选中的用户？', {icon: 3, title: '提示信息'}, function (index) {
	               ...
	               /*  tableIns.reload();
	                ...
	                $.ajax({
						type: "post",
						url: "/StudentServlet/DeleteStudent",
						data: {ids:ids},
